...
import {
  ...
  copyReportsCsv,
  downloadReportsCsv,
  createReportsGoogleSheet,
  toggleReportColumn,
  selectAllReportColumns,
  resetDefaultReportColumns
} from "./reports.js";
...
function toggleReportColumnAndRender(key) {
  toggleReportColumn(key);
  render();
}

function selectAllReportColumnsAndRender() {
  selectAllReportColumns();
  render();
}

function resetDefaultReportColumnsAndRender() {
  resetDefaultReportColumns();
  render();
}
...
async function createReportsGoogleSheetAndRender() {
  try {
    await createReportsGoogleSheet();
    render();
  } catch (error) {
    state.reports.message = error.message || "Could not create Google Sheet.";
    render();
  }
}
...
window.openJobClockLink = (eventId) => {
  const match = state.eventsData.find(
    (e) => String(e.EventId) === String(eventId)
  );
  ...
  const url = match.JobLink || match.JobUrl || match.ClockLink;

  if (!url) {
    alert("No job link found.");
    return;
  }
...
window.toggleReportColumnAndRender = toggleReportColumnAndRender;
window.selectAllReportColumnsAndRender = selectAllReportColumnsAndRender;
window.resetDefaultReportColumnsAndRender = resetDefaultReportColumnsAndRender;



window.copyReportsCsv = copyReportsCsv;
window.downloadReportsCsv = downloadReportsCsv;


window.createReportsGoogleSheetAndRender =
  createReportsGoogleSheetAndRender;
