...
function ensureEventBuilderDefaults() {
  if (!state.eventBuilder) {
    state.eventBuilder = {
      fields: { ...DEFAULTS },
      workers: [
        { workerId: "[WORKER_ID]", workerName: "[WORKER_NAME]", payType: "hourly", rate: "0.00", adjustment: "0" },
        { workerId: "[WORKER_ID]", workerName: "[WORKER_NAME]", payType: "flat", rate: "0.00", adjustment: "0" }
      ],
      titleText: "",
      notesText: "",
      copyMessage: ""
  ...
  if (!state.eventBuilder.fields) state.eventBuilder.fields = { ...DEFAULTS };
  if (!state.eventBuilder.workers) state.eventBuilder.workers = [];
}
...
export function setEventBuilderWorker(index, key, value) {
  ensureEventBuilderDefaults();
  if (!state.eventBuilder.workers[index]) return;
  state.eventBuilder.workers[index][key] = value;
  state.eventBuilder.copyMessage = "";
  generateEventBuilderOutput();
}
  ...
  return `
    <div class="detail-section-card">
      <div class="flex items-center justify-between gap-3 mb-3">
        ...
        <button class="secondary-btn" type="button" onclick="window.removeEventBuilderWorkerAndRender(${index})">
          Remove
        </button>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
        <input class="column-filter-input" value="${escapeHtml(worker.workerId || "")}" placeholder="Worker ID"
          oninput="window.setEventBuilderWorkerAndRender(${index}, 'workerId', this.value)" />

        <input class="column-filter-input" value="${escapeHtml(worker.workerName || "")}" placeholder="Worker Name"
          oninput="window.setEventBuilderWorkerAndRender(${index}, 'workerName', this.value)" />

        <select class="column-filter-select"
          onchange="window.setEventBuilderWorkerAndRender(${index}, 'payType', this.value)">
          <option value="hourly" ${worker.payType === "hourly" ? "selected" : ""}>hourly</option>
          <option value="flat" ${worker.payType === "flat" ? "selected" : ""}>flat</option>
        </select>

        <input class="column-filter-input" value="${escapeHtml(worker.rate || "")}" placeholder="Rate / Flat"
          oninput="window.setEventBuilderWorkerAndRender(${index}, 'rate', this.value)" />

        <input class="column-filter-input" value="${escapeHtml(worker.adjustment || "")}" placeholder="Adjustment"
