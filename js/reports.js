...
function getAuthorizedAssignedHours(row) {
  ...
  const baseHours = getJobAssignedHours(row);

  const extraMatch = text.match(/\(([+-]?\d+)\s*m\)/i);
  const extraMinutes = extraMatch ? Number(extraMatch[1]) : 0;

  return baseHours + extraMinutes / 60;
}
...
function getJobPayout(row, workerId) {
  ...
  if (payType === "hourly") {
    ...
    return payableWorkedHours * rate;
  }
