...
function buildTravelEventBody() {
  ...
  const mileageRateNumber = Number(mileageRate || 0);

  const driveTimePay =
    summary.payableMinutes > 0
      ? (summary.payableMinutes / 60) * hourlyRateNumber
      : 0;

  const mileagePay =
    summary.totalMiles > 0
      ? summary.totalMiles * mileageRateNumber
      : 0;

  const totalDrivePay = driveTimePay + mileagePay;
  ...
  lines.push(`TOTAL_DRIVE_TIME_PAY: $${driveTimePay.toFixed(2)}`);
  lines.push(`TOTAL_MILEAGE_REIMBURSEMENT_PAY: $${mileagePay.toFixed(2)}`);
  lines.push(`TOTAL_DRIVE_PAY: $${totalDrivePay.toFixed(2)}`);
  lines.push("");
  lines.push("FORMULA:");
  lines.push("PAYABLE_DRIVE_MINUTES = MAX(TOTAL_DRIVE_TIME_MINUTES - (FREE_MINUTES_PER_LEG × LEG_COUNT), 0)");
  lines.push("TOTAL_DRIVE_TIME_PAY = (PAYABLE_DRIVE_MINUTES / 60) × HOURLY_RATE");
  lines.push("TOTAL_MILEAGE_REIMBURSEMENT_PAY = TOTAL_MILES × MILEAGE_RATE");
  lines.push("TOTAL_DRIVE_PAY = TOTAL_DRIVE_TIME_PAY + TOTAL_MILEAGE_REIMBURSEMENT_PAY");
  lines.push("");
