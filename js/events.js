...
function buildTravelLogText() {
  ...
  const workerRole = state.routeBuilder.travelLogWorkerRole || "";
  ...
  lines.push("TRAVEL_LOG_VERSION: 1");
  ...
  lines.push("");
  lines.push("TITLE:");
  lines.push(`TRAVEL LOG | ${workerName || "Worker"} | ${calendarName} | ${date}`);
  lines.push("");
  ...
  if (stops.length < 2) {
    lines.push("No travel legs found. At least two stops are needed.");
  } else {
    for (let i = 0; i < stops.length - 1; i++) {
      ...
      lines.push("MILES:");
      lines.push("DRIVE_TIME_MINUTES:");
      lines.push("FREE_MINUTES: 30");
      lines.push("PAID_DRIVE_MINUTES:");
      lines.push("MILEAGE_PAY:");
      lines.push("DRIVE_TIME_PAY:");
      lines.push("NOTES:");
    }
  }
  ...
  lines.push("");
  lines.push("SUMMARY");
  lines.push("TOTAL_MILES:");
  lines.push("TOTAL_DRIVE_TIME_MINUTES:");
  lines.push("TOTAL_PAID_DRIVE_MINUTES:");
  lines.push("TOTAL_MILEAGE_PAY:");
  lines.push("TOTAL_DRIVE_TIME_PAY:");
  lines.push("TOTAL_TRAVEL_PAY:");
  lines.push("");
  lines.push("INSTRUCTIONS:");
  lines.push("Use this event as the travel/payroll holder for the worker's route.");
  lines.push("Drive time pay applies only to minutes exceeding 30 minutes per leg.");
...
export function setTravelLogWorkerRole(value) {
  state.routeBuilder.travelLogWorkerRole = value;
}
...
export function generateTravelLogForRouteBuilder() {
  ...
  if (stops.length < 2) {
    state.routeBuilder.copyMessage = "At least two stops are needed to generate a travel log.";
    state.routeBuilder.travelLogText = "";
    return;
  }

  state.routeBuilder.travelLogText = buildTravelLogText();
  state.routeBuilder.copyMessage = "Travel log generated.";
}

export async function copyTravelLog() {
  const text = state.routeBuilder.travelLogText || buildTravelLogText();

  try {
    await navigator.clipboard.writeText(text);
    state.routeBuilder.travelLogText = text;
    state.routeBuilder.copyMessage = "Travel log copied. Paste it into the calendar event description.";
  } catch (_) {
  ...
  return `
    <div class="panel">
      <div class="panel-header">
        ...
        <p class="text-sm text-slate-500 mt-1">
          Build a route for one calendar and date, then generate a copy-ready travel log for payroll.
        </p>
      </div>
      ...
      <div class="panel-body space-y-5">
        ...
        <div class="detail-section-card">
          ...
          <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <input
              ...
              oninput="window.setTravelLogWorkerName(this.value)"
            />
            <input
              class="column-filter-input"
              placeholder="Worker ID"
              value="${escapeHtml(state.routeBuilder.travelLogWorkerId || "")}"
              oninput="window.setTravelLogWorkerId(this.value)"
            />
            <select
              class="column-filter-select"
              onchange="window.setTravelLogWorkerRole(this.value)"
            >
              <option value="">Role</option>
              <option value="Driver" ${state.routeBuilder.travelLogWorkerRole === "Driver" ? "selected" : ""}>Driver</option>
              <option value="Passenger" ${state.routeBuilder.travelLogWorkerRole === "Passenger" ? "selected" : ""}>Passenger</option>
              <option value="Team Lead" ${state.routeBuilder.travelLogWorkerRole === "Team Lead" ? "selected" : ""}>Team Lead</option>
              <option value="Assistant" ${state.routeBuilder.travelLogWorkerRole === "Assistant" ? "selected" : ""}>Assistant</option>
            </select>
            <input
              class="column-filter-input"
              placeholder="Hourly Rate"
              value="${escapeHtml(state.routeBuilder.travelLogHourlyRate || "")}"
              oninput="window.setTravelLogHourlyRate(this.value)"
            />
            <input
              class="column-filter-input"
              placeholder="Mileage Rate"
              value="${escapeHtml(state.routeBuilder.travelLogMileageRate || "0.70")}"
              oninput="window.setTravelLogMileageRate(this.value)"
            />
          </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <button
            class="primary-btn"
            type="button"
            onclick="window.generateTravelLogAndRender()"
          ...
          <button
            class="secondary-btn"
            type="button"
            onclick="window.copyTravelLogAndRender()"
            ${stops.length >= 2 ? "" : "disabled"}
          >
            Copy Travel Log
          </button>
        ...
        <div class="detail-section-card">
          ...
          ${
            travelLogText
              ? `<textarea class="toolbar-textarea" rows="18" readonly>${escapeHtml(travelLogText)}</textarea>`
              : `<div class="text-slate-500">Generate a travel log to preview the copy-ready text.</div>`
          }
        </div>
      </div>
    </div>
