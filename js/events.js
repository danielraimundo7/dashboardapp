...
function buildTravelEventBody() {
  ...
  lines.push("TRAVEL_LOG_VERSION: 2");
  ...
  lines.push("Current rule: 25 non-payable minutes are allotted per leg. Only the remaining total minutes are payable.");
...
export function selectTravelLogWorkerFromSearch(value) {
  ...
  if (transportation) {
    state.routeBuilder.travelLogWorkerRole = transportation;
  }
...
export async function copyTravelLogTitle() {
  try {
    await navigator.clipboard.writeText(buildTravelEventTitle());
...
export async function copyTravelLogBody() {
  try {
    await navigator.clipboard.writeText(buildTravelEventBody());
    ...
    <div class="panel">
      ...
      <div class="panel-body space-y-5">
        ...
        <div class="detail-section-card">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <div class="stat-label">Role / Transportation</div>
              <input
                class="column-filter-input"
                placeholder="Driver / Passenger"
                value="${escapeHtml(state.routeBuilder.travelLogWorkerRole || "")}"
        ...
        <div class="detail-section-card">
          ...
          <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              <div class="stat-label">Leg Count</div>
        ...
        <div class="detail-section-card">
          ...
          <div class="grid grid-cols-1 gap-4">
            ...
            <div class="flex flex-wrap gap-2">
              <button class="secondary-btn" type="button" onclick="window.copyTravelLogTitleAndRender()" ${stops.length >= 2 ? "" : "disabled"}>
                Copy Event Title
              </button>

              <button class="secondary-btn" type="button" onclick="window.copyTravelLogBodyAndRender()" ${stops.length >= 2 ? "" : "disabled"}>
                Copy Event Body
              </button>

              <button class="primary-btn" type="button" onclick="window.generateTravelLogAndRender()" ${stops.length >= 2 ? "" : "disabled"}>
                Generate Preview
              </button>
            </div>
          </div>
