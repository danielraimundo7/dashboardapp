...
function getElapsedMinutes(startValue) {
  const start = toDate(startValue);
  if (!start) return 0;
...
function getLiveTimeFlags(row) {
  const assignedHours =
    parseHoursValue(row.AssignedTime);

  const adjustmentHours =
    parseHoursValue(row.AuthorizedTimeAdjustment);

  const allowedHours =
    Math.max(assignedHours + adjustmentHours, 0);
  ...
  const elapsedMinutes =
    getElapsedMinutes(row.ClockInTime);

  if (elapsedMinutes > allowedMinutes + 30) {
    return [
      `<span class="badge badge-danger">Very Long</span>`
    ];
  }
...
function renderLiveFlags(row) {
  const flags = getLiveTimeFlags(row);
        ...
        <div class="table-scroll-wrap">
          <table>
            ...
            <tbody>
              ${
                openRows.length === 0
                  ? `<tr><td colspan="8" class="text-center text-slate-500">No workers are currently clocked in.</td></tr>`
